...
const int terrainVoxelNum = terrainVoxelWidth * terrainVoxelWidth; //地形体素点云数量
...
float vehicleX = 0, vehicleY = 0, vehicleZ = 0;  //odom中的机器人三维坐标
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(  //雷达发布的pcl格式的点云数据，话题为/points_raw
  ...
  downSizeFilter.setLeafSize(scanVoxelSize, scanVoxelSize, scanVoxelSize);  //设置点云下采样滤波器
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      // terrain voxel roll over 地形体素点云滚动
      ...
      for (int i = 0; i < laserCloudCropSize; i++) {  //地形体素点云地图的构建
      ...
      if (useSorting) {
        ...
        for (int i = 0; i < planarVoxelNum; i++) {
          ...
          if (planarPointElevSize > 0) {
            float minZ = 1000.0;
            int minID = -1;
            for (int j = 0; j < planarPointElevSize; j++) {  //循环找到该体素网格中的最小高程
